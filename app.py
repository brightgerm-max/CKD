    ...
    if price_cache_key not in st.session_state:
        with st.spinner("자사 및 경쟁사 채널별 가격을 조회하고 있습니다..."):
            ...
            for comp in db_comps:
                ...
                api_prices = search_product_prices(search_kw, brand_keywords=[c_brand])
                # 수동 가격이 있으면 오버라이드
        ...
        with st.expander(f"📁 {cat_name} ({len(comps)}개)", expanded=False):
            ...
            with st.form(f"cdb_add_{cat_name}"):
                ...
                with a2:
                    pass
                # 성분 / 건강기능
                st.markdown('<div style="font-size:0.85rem;font-weight:700;color:var(--c-text);margin:12px 0 2px">성분 / 건강기능 표시</div>', unsafe_allow_html=True)
                ...
                # USP / 셀링포인트
                st.markdown('<div style="font-size:0.85rem;font-weight:700;color:var(--c-text);margin:12px 0 2px">USP / 셀링포인트</div>', unsafe_allow_html=True)
                ...
                # 채널별 URL
                st.markdown('<div style="font-size:0.85rem;font-weight:700;color:var(--c-text);margin:12px 0 2px">채널별 URL</div>', unsafe_allow_html=True)
                ...
                # 채널별 가격
                st.markdown('<div style="font-size:0.85rem;font-weight:700;color:var(--c-text);margin:12px 0 2px">채널별 가격 <span style="font-weight:400;color:#94a3b8;font-size:0.72rem">(선택)</span></div>', unsafe_allow_html=True)
                st.caption("단위 입력 형식: 60정, 120캡슐, 30포, 2개월분, 90일분")
            ...
            if comps:
                ...
                # 수정 시 URL 자동추출
                st.markdown('<div style="font-size:0.85rem;font-weight:700;color:var(--c-text);margin:8px 0 2px">자동 추출</div>', unsafe_allow_html=True)
                ...
                with st.form(f"cdb_edit_{cat_name}_{sel_idx}"):
                    ...
                    st.markdown('<div style="font-size:0.85rem;font-weight:700;color:var(--c-text);margin:12px 0 2px">채널별 가격 <span style="font-weight:400;color:#94a3b8;font-size:0.72rem">(선택, 0=API 자동)</span></div>', unsafe_allow_html=True)
                    st.caption("단위 입력 형식: 60정, 120캡슐, 30포, 2개월분, 90일분")
                    ep1, ep2, ep3 = st.columns(3)
                    with ep1:
